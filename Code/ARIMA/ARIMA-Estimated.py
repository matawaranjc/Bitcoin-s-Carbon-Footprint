...
# Fit ARIMA model
model = sm.tsa.ARIMA(train['Coal-only'], order=(1, 1, 1))
model_fit = model.fit()
...
# Plot the results
ax = train['Coal-only'].plot(figsize=(15, 5), title='Coal-only: Greenhouse Gas Emissions')
test['Coal-only'].plot(ax=ax)
predictions_next_year.plot(ax=ax)
plt.legend(['Historical Data', 'Test Set', 'Predictions'])
plt.show()

# Calculate RMSE on the test set
rmse = np.sqrt(mean_squared_error(test['Coal-only'], predictions_next_year))
